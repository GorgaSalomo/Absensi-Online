...
class LoginActivity : Activity() {
    ...
    private val userRoleString = arrayOf("admin", "Dosen")
        ...
        login!!.setOnClickListener { // TODO Auto-generated method stub
            if (userrole == "admin") {
                ...
                if (TextUtils.isEmpty(user_name)) {
                    username!!.error = "User Name Gagal"
                ...
                val facultyBean: LecturerBean = dbAdapter.validateFaculty(user_name, pass_word)
                if (facultyBean != null) {
                    val intent = Intent(this@LoginActivity, AddAttandanceSessionActivity::class.java)
                    startActivity(intent)
                    (this@LoginActivity.applicationContext as ApplicationContext).setLecturerBean(facultyBean)
                    Toast.makeText(applicationContext, "Login Berhasil", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(applicationContext, "Login Gagal", Toast.LENGTH_SHORT).show()
                }
